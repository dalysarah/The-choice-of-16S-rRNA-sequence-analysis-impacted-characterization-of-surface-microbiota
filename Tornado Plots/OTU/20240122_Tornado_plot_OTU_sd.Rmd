---
title: "Tornado Plot Environmental Sampling-OTU Approach"
author: "Sarah Daly"
date: '2023-08-25'
output:
...
```{r}
setwd("C:/Users/dalys/OneDrive/Documents/My Documents/Cornell Postdoc Papers/Review Paper/Manuscript/Code/Tornado Plots/OTU")
```
...
# Importing metadata
metadata <- read.table("Metadata_tornado.txt", sep='\t', header=T, row.names=1, comment="")
head(metadata)
dim(metadata)
...
#Filter samples you want
```{r}
ps <- subset_samples(ps, keep==TRUE)
...
tiff(paste(name, "Mean Compositional Abundance Genus single bar OTU.tif", sep=" "), res=300, compression = "lzw", height=9, width=15, units="in")
print(bar)
dev.off()

pdf(paste(name, "Mean Compositional Abundance Genus single bar OTU.pdf", sep=" "), height=9, width=15)
print(bar)
dev.off()
